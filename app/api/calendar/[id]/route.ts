...
export async function GET(
  ...
  return NextResponse.json({
    ...
    labels: ev.labels,
    assets: ev.attachments.map((a: { assetId: string }) => ({
      id: a.assetId,
  ...
  const updated = await prisma.$transaction(
    async (tx: Prisma.TransactionClient) => {
      ...
      if (Array.isArray(json.assetIds)) {
        await tx.calendarEventAsset.deleteMany({
          where: { eventId: id },
        });

        if (json.assetIds.length) {
          await tx.calendarEventAsset.createMany({
            data: json.assetIds.map((aid: string) => ({
              eventId: id,
              assetId: aid,
            })),
          });
